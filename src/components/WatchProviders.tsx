...
export function WatchProviders({ providers, size = "sm", subscribedIds, showName }: WatchProvidersProps) {
  ...
  const imgClass = size === "sm" ? "w-5 h-5 rounded-[4px]" : "w-8 h-8 rounded-md";
      ...
      {shown.map((p) => {
        ...
        const img = (
          <Image
            ...
            className={`${imgClass} ${dimmed ? "opacity-30 grayscale" : ""} ${url ? "hover:ring-2 hover:ring-white/50 transition-all" : ""}`}
            title={url ? `Watch on ${p.provider_name}` : `${p.provider_name}${dimmed ? " (not subscribed)" : ""}`}
          />
        );
        ...
        return url ? (
          ...
          <span key={p.provider_id}>{img}</span>
        );
      })}
      {overflow > 0 && (
